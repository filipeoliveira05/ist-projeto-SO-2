...
void cleanup_fifos(const char *req_pipe_path, const char *resp_pipe_path,
...
int kvs_connect(char const *req_pipe_path, char const *resp_pipe_path,
  ...
  memcpy(buf + 1 + 2 * MAX_PIPE_PATH_LENGTH, notif_pipe_path,
  ...
  client->req_pipe =
      open(req_pipe_path, O_RDWR); // Open for both read and write
...
int kvs_disconnect(Client *client) {
  ...
  do {
    bytes_read =
        read(client->resp_pipe, buf, 2); // Read the response (2 bytes expected)
  ...
  cleanup_fifos(client->req_pipe_path, client->req_pipe_path,
